...
left_line = lane.Line(n=window_size)
right_line = lane.Line(n=window_size)
...
def annotate_image(img_in):
	...
	img, abs_bin, mag_bin, dir_bin, hls_bin = threshold.imageThreshold(undist)
	binary_warped, binary_unwarped, m, m_inv = perspectiveTransform.perspectiveTransform(img)

	# Perform polynomial fit
	if not detected:
		# Slow line fit
		ret = laneFinder.laneLineFit(binary_warped)
		...
		# Calculate curvature
		left_curve, right_curve = laneFinder.calc_curve(left_lane_inds, right_lane_inds, nonzerox, nonzeroy)
		...
		ret = laneFinder.tune_fit(binary_warped, left_fit, right_fit)
		left_fit = ret['left_fit']
		right_fit = ret['right_fit']
		nonzerox = ret['nonzerox']
		nonzeroy = ret['nonzeroy']
	...
	vehicle_offset = laneFinder.calc_vehicle_offset(undist, left_fit, right_fit)

	# Perform final visualization on top of original undistorted image
	result = laneFinder.final_viz(undist, left_fit, right_fit, m_inv, left_curve, right_curve, vehicle_offset)
...
if __name__ == '__main__':
	# Annotate the video
	annotate_video('challenge_video.mp4', 'out.mp4')
